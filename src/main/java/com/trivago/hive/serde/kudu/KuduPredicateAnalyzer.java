...
public class KuduPredicateAnalyzer {
  ...
  public static DecomposedPredicate decomposePredicate(JobConf jobConf, ExprNodeDesc predicate) {
    ...
    String[] keyColumns = jobConf.get(HiveKuduConstants.KEY_COLUMNS).split("\\s*,\\s*");
    ...
    try {
      for (IndexSearchCondition indexSearchCondition : conditions) {
        ...
        Type kuduType = HiveKuduBridgeUtils.hiveTypeToKuduType(indexSearchCondition.getColumnDesc().getTypeInfo().getTypeName());
        ColumnSchemaBuilder columnSchemaBuilder = new ColumnSchema.ColumnSchemaBuilder(
        ...
        if (kuduType.equals(Type.DECIMAL)) {
          ...
          ColumnTypeAttributes decimalAttr = new ColumnTypeAttributesBuilder().precision(precision).scale(scale).build();
