...
public class HiveKuduBridgeUtils {
  ...
  public static ObjectInspector getObjectInspector(ColumnSchema kuduColumn) throws SerDeException {
    switch (kuduColumn.getType()) {
      ...
      case DECIMAL: {
        PrimitiveTypeInfo typeInfo = new DecimalTypeInfo(kuduColumn.getTypeAttributes().getPrecision(), kuduColumn.getTypeAttributes().getScale());
  ...
  public static SerDeStats convertStatistics(Statistics stats) {
    SerDeStats outStats = new SerDeStats();
    outStats.setRowCount(stats.getClientStatistic(Statistic.WRITE_OPS));
    outStats.setRawDataSize(stats.getClientStatistic(Statistic.BYTES_WRITTEN));
    return outStats;
  }
}
